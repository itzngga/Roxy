...
export default class Exif {
   ...
   */
  public static create(packname: string, authorname: string, filename: string) {
    ...
    let len = new TextEncoder().encode(JSON.stringify(json)).length as any;
    const f = Buffer.from([
    ...
    fs.writeFile(`./temp/${filename}.exif`, buffer, (err: any) => {
